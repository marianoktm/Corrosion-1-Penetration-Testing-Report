...
\section {Overview}
This chapter highlights how the findings were identified and classified through both manual and automated analysis. In addition, the rating system is defined.

\section {Summary}
Automated analysis conducted with Nessus and Nikto discovered 81 vulnerabilities of the following severity: 20 Critical, 40 High, 18 Low. All of these vulnerabilities are related to an outdated version of the Apache2 HTTP Web Server running on the target machine.
\\

Manual analysis found 7 additional vulnerabilities of the following severity: 1 Critical, 5 High, 1 Medium. These vulnerabilities are related to Web Application information exposure, log poisoning, weak passwords, weak permission policies and missing OS security patches.
\\

Vulnerabilities are rated according to CVSS 3.0:

\begin{table}[htbp]
	\centering
	\caption{Vulnerability Severity Classification (CVSS 3.0)}
	\label{tab:vulnerability-severity}
	\begin{tabular}{@{}llp{10cm}@{}}
		\toprule
		\textbf{Severity} &
		\textbf{CVSS v3.0} &
		\textbf{General Risk Description}\\
		\midrule
		
		Critical &
		9.0--10.0 &
		Severe risk with potential for major exploitation.\\
		
		High &
		7.0--8.9 &
		Significant risk of high-level compromise.\\
		
		Medium &
		4.0--6.9 &
		Moderate risk with limited or conditional security impact. \\
		
		Low &
		0.1--3.9 &
		Limited security impact and/or low likelihood of successful
		exploitation. \\
...
\section{Overview}
This chapter presents the findings of the assessment in greater detail. Each identified vulnerability is documented with its corresponding severity level, CVE reference, and CVSS score, technical descriptions, and targeted remediation strategies.
\\
